...
export const ALL_VARIABLES: VariableDefinition[] = [
  ...
  { name: "qrcode_inscricao", label: "QR code da inscrição (anexo)", category: "Inscrição", description: "Anexa a imagem do QR code de retirada do kit (mesmo código da tela de retirada, gerado a partir de Registration.id). Ao usar esta variável, a mensagem inteira é enviada como imagem, com o restante do texto como legenda — o token em si não aparece como texto. Legendas de imagem no WhatsApp têm limite menor que mensagens de texto (~1024 caracteres) — evite mensagens muito longas junto desta variável.", sample: "[a mensagem será enviada como imagem, com este texto como legenda]" },
  ...
  { name: "taxa_servico", label: "Taxa de serviço (líquida)", category: "Plataforma", description: "Soma de Order.paymentFeeAmount no período (Taxa de Serviço já líquida, com o desconto PIX aplicado quando houver), formatada em R$. Só disponível no resumo diário do administrador.", sample: "R$ 45,00" },
  { name: "taxa_servico_bruta", label: "Taxa de serviço (bruta)", category: "Plataforma", description: "Soma de Order.serviceFeeOriginalAmount no período (Taxa de Serviço ANTES do desconto PIX), formatada em R$. Só disponível no resumo diário do administrador.", sample: "R$ 50,00" },
  { name: "desconto_pix", label: "Descontos PIX concedidos", category: "Plataforma", description: "Soma de Order.pixDiscountAmount no período (total de desconto PIX que a plataforma absorveu na Taxa de Serviço), formatada em R$ com sinal negativo quando > 0. Só disponível no resumo diário do administrador. Não afeta a Taxa da Plataforma nem o repasse do organizador.", sample: "-R$ 5,00" },
