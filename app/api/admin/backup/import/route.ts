...
function toAthleteProfileRow(row: Row): Prisma.AthleteProfileCreateManyInput {
  return {
    ...
    postalCode: sn(row.postalCode),
    street: sn(row.street),
    number: sn(row.number),
    complement: sn(row.complement),
    neighborhood: sn(row.neighborhood),
    city: sn(row.city),
    state: sn(row.state),
...
function toEventRow(row: Row): Prisma.EventCreateManyInput {
  return {
    ...
    registrationEditDeadline: dn(row.registrationEditDeadline),
...
function toRegistrationRow(row: Row): Prisma.RegistrationCreateManyInput {
  return {
    ...
    participantName: s(row.participantName),
    participantEmail: s(row.participantEmail),
    participantPhone: sn(row.participantPhone),
    participantBirthDate: dn(row.participantBirthDate),
    participantGender: sn(row.participantGender),
    participantCpf: sn(row.participantCpf),
