  ...
  return (
    <main className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">{total} evento{total !== 1 ? "s" : ""} encontrado{total !== 1 ? "s" : ""}</p>
      </div>
                ...
                <div className="flex justify-center mt-8 gap-2">
                  {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
                    <a
                      key={p}
                      href={`?pagina=${p}`}
                      className={`px-3 py-1 rounded ${p === page ? "bg-primary-600 text-white" : "bg-white border"}`}
                    >
                      {p}
                    </a>
                  ))}
