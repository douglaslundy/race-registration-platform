...
// cancelPendingPaymentManually pra testar só a lógica da rota em isolamento), este arquivo deixa
...
//
// O gateway (getPaymentProvider) é mockado — cancelar de verdade no Mercado Pago/Pagar.me está
// fora do escopo deste teste, que é sobre o efeito local (Registration/TicketBatch/notificação).
...
describe("cancelamento manual do organizador bloqueia o QR code de retirada de kit", () => {
  ...
  it("grava status CANCELLED na Registration, libera a vaga do lote e dispara a notificação de pagamento não identificado (cancelExpiredPayment real, sem mock)", async () => {
    ...
    // Lookup que cancelPendingPaymentManually faz antes de tudo, pra saber com qual
    // providerPaymentId cancelar no gateway.
    dbMock.payment.findUnique.mockResolvedValueOnce({ status: "PENDING", providerPaymentId: "mp-1" });
    ...
    // Cancela no gateway ANTES de tocar no banco local (Fix crítico: sem isso, o PIX continuaria
    // pagável no Mercado Pago por até ~20h depois deste cancelamento manual).
    expect(cancelPaymentGateway).toHaveBeenCalledWith("mp-1");
